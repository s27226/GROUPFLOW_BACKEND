...
public class S3Service : IS3Service
{
    ...
    public async Task<string> UploadFileAsync(Stream stream, string fileName, string contentType, string blobPath)
    {
        try
        {
            ...
            _logger.LogInformation("Uploaded file to S3: {BlobPath}", blobPath);
            return blobPath;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error uploading file to S3: {BlobPath}", blobPath);
            throw;
        }
    }

    public Task<string> GetPresignedUrlAsync(string blobPath, int expiryInSeconds = 604800)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, "Error getting presigned URL for: {BlobPath}", blobPath);
            throw;
        }
    }
    ...
    public async Task DeleteFileAsync(string blobPath)
    {
        try
        {
            ...
            _logger.LogInformation("Deleted file from S3: {BlobPath}", blobPath);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error deleting file from S3: {BlobPath}", blobPath);
            throw;
        }
    }
    ...
    public async Task<(long size, string contentType)> GetFileMetadataAsync(string blobPath)
    {
        ...
        {
            _logger.LogError(ex, "Error getting file metadata for: {BlobPath}", blobPath);
            throw;
        }
    }
}
